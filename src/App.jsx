...
import traﬀyLogo from "./assets/traffy.png";
...
export default function App() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        // เปิดโหมด login อัตโนมัติสำหรับเบราว์เซอร์ภายนอก
        await liff.init({
          liffId: LIFF_ID,
          withLoginOnExternalBrowser: true,
        });

        // ถ้ายังไม่ล็อกอิน ให้ login ทันที
        if (!liff.isLoggedIn()) {
          liff.login();
          return;
        }
  ...
  return (
    <div className="chatwin" role="dialog" aria-label="Traffy Fondue Chat Support">
      <div className="chat-header">
        <img src={traﬀyLogo} alt="Traffy Fondue Logo" className="chat-logo" loading="lazy" />
        <div>
      ...
      <form className="inputbar" onSubmit={onSubmit}>
        <textarea
          rows={1}
          placeholder="พิมพ์คำถาม ที่นี่..."
          value={input}
          onChange={(e) => setInput(e.target.value)}
          onKeyDown={onKeyDown}
        />
        <button type="submit" className="send-btn" aria-label="ส่ง">
